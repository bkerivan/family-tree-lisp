...
(DEFUN handle-X (names tree)
  "NAMES is a LIST of strings. TREE is a hash-table."
  (LET ((relation (nth 1 names))
        (name1 (nth 0 names))
        (name2 nil)
        (degree -1)
        (query nil))

    (cond ((eql (list-length names) 3) (setq name2 (nth 2 names)))
          ((eql (list-length names) 4) (setq name2 (nth 3 names) degree (parse-integer (nth 2 names))))
    ...
    (case relation
      ("ancestor" (setq query (is-ancestor name1 name2 tree)))
      ("unrelated" (setq query (is-unrelated name1 name2 tree)))
      ("child" (setq query (is-child name1 name2 tree)))
      ("sibling" (setq query (is-sibling name1 name2 tree)))
      ("cousin" (setq query (is-cousin name1 name2 tree degree)))
      (otherwise (error "HANDLE-X: Invalid relation: ~A" relation)))

    query
    ))


(DEFUN handle-W (names tree)
  "NAMES is a LIST of strings. TREE is a hash-table."
  (LET ((relation (nth 0 names))
        (name nil)
        (degree -1)
        (query nil))

    (cond ((eql (list-length names) 2) (setq name (nth 1 names)))
          ((eql (list-length names) 3) (setq name (nth 2 names) degree (parse-integer (nth 1 names))))
    ...
    (case relation
      ("ancestor" (setq query (get-ancestors name tree)))
      ("unrelated" (setq query (get-unrelated name tree)))
      ("child" (setq query (get-children name tree)))
      ("sibling" (setq query (get-siblings name tree)))
      ("cousin" (setq query (get-cousins name tree degree)))
      (otherwise (error "Invalid relation: ~A" relation)))

    (when query
        (setq query (sort query #'string-lessp)))
